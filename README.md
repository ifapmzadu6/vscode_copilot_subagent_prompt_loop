...
AIモデル（LLM）から最良の結果を引き出すためには、適切な「プロンプト」を作成することが不可欠ですが、どのような指示が最も効果的かは試行錯誤が必要です。

このプロジェクトの目的は、**プロンプトエンジニアリングのプロセスを自動化・最適化すること**です。

具体的には、以下の課題を解決します：
1. **手作業による試行錯誤の削減**: ユーザーが何度も書き直すのではなく、AIが自動的に複数のアプローチを試します。
2. **最適なアプローチの発見**: 「段階的に考えて」「専門家として振る舞って」など、異なる戦略を並行して実行し、タスクに最適な指示方法を特定します。
3. **継続的な改善**: 結果を分析し、より良い結果を得るための改善点を学習・適用します。

これにより、ユーザーは「何をしてほしいか」というタスクを投げるだけで、最も効果的な結果と、それを導き出すための最適なプロンプトの両方を得ることができます。
...
| `iterations` | No | 最適化ループの回数（デフォルト: 20） |
